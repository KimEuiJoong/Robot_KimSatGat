...
with tf.device('/device:CPU:0'):
    ...
    vocab_size_token = 4337 #미리 계산
    vocab_size_str = 2494 #미리 계산
    ...
    tags11 = ['기쁨','사랑1','순수','잔잔','슬픔','의지','그리움1','그리움2','고독','성찰']
    ...
    def model_control(l, o, e, a, d, x, y):
        ...
        layers = [5, 10] #1
        optimizers = ['adam', 'sgd', 'Adadelta', momentum] #1
        embedding_sizes = [100, 10, 200] #1
        activations = ['softsign', 'relu', swish, 'elu', 'tanh'] #1
        dropouts = [0.5, 0.2] #1
        vocab_sizes = {'s':vocab_size_str, 't':vocab_size_token} #1
        tag_numbers = [16, 10] #1
        #max_len = 200
        ...
        tnan = TerminateOnNaN()
        es = EarlyStopping(monitor='val_loss', mode='min', min_delta=0,  verbose=1, patience=25) #과적합방지
